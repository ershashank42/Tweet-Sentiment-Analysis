...
if __name__ == "__main__":
    ...
    '''
    #Get average length of all tweets extracted
    #print(np.mean(df['len'])) #self-expainatory

    #get the number of likes for the most liked tweets
    #print(np.max(df['likes']))

    #time series for likes - plot that will show us number of lets say like recived by tweets of
    #donald trump on any given day over a course of some days
    ...
    #we can also plot two graphs in same plot
    time_likes = pd.Series(data=df['likes'].values, index=df['date'])
...
that one block is one tweet which is a JSON formatted dictionary object and each of the field contains info about the tweet.
eg. date, id of the tweet, tweeted or re-tweeted by which person and the actual keyword because of which it showed the tweet
in the list.


Part 2: we'll focus on page notion and we'll focus on cursor object in tweepy library.
we'll use that to access own tweets on our timeline and accessing user tweets from specific user that we might to analyze
tweets from, also getting followers and friends from certain users etc.

Twitter API imposes things like rates of limits - if you're throttling  the twitter API and trying to get all the
information then twitter might stop you from being abusing the system. If twitter thinks that you're doing This
continuously you'll get error msgs. Fist time, you'll get error and you have to wait to access the tweets again
but if u keep ignoring those msgs then that window time increases exponentially and you lock yourself from being
accessing the information


twitter API documentation read
tweepy.API documentation
TextBolb documentation read

part 3: analyzing the tweets using pandas and numpy pandas will give us content that will allows us to store
tweets and corresponding things that we extract from those tweets into data frame numpy is general purpose numerical library

part 4: visualization of tweet data: time series graph based on dsta we stored in dataframe

part 5: sentiment analysis - decide whether a tweet is positive, negative or neutral. For this purpose we'll use a module
called text blob, this module has an inbuilt sentiment analyzer trained on data'''
